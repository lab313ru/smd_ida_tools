...
static void define_vectors_struct()
{
	...
	tid_t vec_id = add_struc(BADADDR, "vec_struc");
	...
	create_struct(0, sizeof(gen_vect), vec_id);
	set_name(0, "vec");
}
...
{
	tid_t head_id = add_struc(BADADDR, "head_struc");
	...
	create_struct(0x100, sizeof(gen_hdr), head_id);
	set_name(0x100, "head");
}

static void define_sprite_struct()
{
	tid_t sprite_id = add_struc(BADADDR, "sprite_struc");
...
static void idaapi load_file(linput_t* li, ushort neflags, const char* fileformatname)
{
	...
	inf.start_ip = inf.start_ea = inf.main = get_name_ea(BADADDR, VECTOR_NAMES[1]); // Reset
